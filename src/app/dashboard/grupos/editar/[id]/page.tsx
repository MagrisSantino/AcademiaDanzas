...
export default function EditarGrupoPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (gData) {
        setNombre(gData.nombre);
        setProfesora(gData.profesora);
        setAlumnasSeleccionadas(gData.alumnas_ids || []);
      }
    };
    fetchData();
  }, [grupoId]);
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 md:p-8 rounded-xl shadow-sm border border-brand-pink space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input type="text" required value={profesora} onChange={e => setProfesora(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-fuchsia outline-none" />
          </div>
        </div>
        ...
        <div className="pt-6 border-t border-gray-100">
          <button type="submit" disabled={loading} className="w-full bg-brand-dark text-brand-light font-bold py-3 rounded-lg hover:bg-gray-800 transition-colors flex justify-center items-center gap-2">
            <Save size={20} /> {loading ? 'Guardando...' : 'Actualizar Grupo'}
          </button>
        </div>
      </form>
    </div>
