...
export default function EstadisticasPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-black text-brand-dark uppercase tracking-tighter">Estadísticas Financieras</h1>
      ...
      <div className="bg-white p-4 rounded-xl border border-brand-pink shadow-sm flex gap-4 items-end">
        <div>
          ...
          <select value={mesFiltro} onChange={e => setMesFiltro(e.target.value)} className="p-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-brand-fuchsia min-w-[150px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-8 rounded-2xl border-l-8 border-green-500 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <p className="font-bold text-gray-500">EFECTIVO</p>
            <Wallet className="text-green-500" />
          </div>
          <p className="text-3xl font-black text-brand-dark">${stats.efectivo.toLocaleString('es-AR')}</p>
        </div>

        <div className="bg-white p-8 rounded-2xl border-l-8 border-blue-500 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <p className="font-bold text-gray-500">TRANSFERENCIA</p>
            <Landmark className="text-blue-500" />
          </div>
          <p className="text-3xl font-black text-brand-dark">${stats.transferencia.toLocaleString('es-AR')}</p>
        </div>

        <div className="bg-brand-fuchsia p-8 rounded-2xl shadow-lg">
          <div className="flex justify-between items-start mb-4 text-brand-dark">
            <p className="font-bold opacity-80">TOTAL INGRESOS</p>
            <TrendingUp />
          </div>
          <p className="text-3xl font-black text-brand-dark">${stats.total.toLocaleString('es-AR')}</p>
        </div>
      </div>

      <div className="bg-gray-100 p-4 rounded-lg flex items-center gap-2 text-gray-600 text-sm">
        <Calendar size={16} />
        <span>Mostrando datos de: <strong>{mesFiltro === 'Todos' ? `Año ${anioFiltro}` : `${mesFiltro} ${anioFiltro}`}</strong></span>
      </div>
    </div>
  );
}